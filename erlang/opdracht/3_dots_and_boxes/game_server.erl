...
% Starts a link between game server and the game.
...
% Initialises the game and tells the first player to make a move.
...
% Checks if the game is finished or not. If so, send finished to every player.
% If not, tell the next player to move.
handle_continue(next_player, {Grid, Players}) ->
    Game_completed = grid:game_completed(Grid),
    [Next|_] = Players,
    if
        Game_completed =:= false -> Next ! {move, self(), Grid};
...
% Adds the wall to the state of the game, if no points have been made, the next
% player is placed first in the playerlist of the sate and continue is called.
% If a point has been made, the order of players stays the same and continue is
% called.
